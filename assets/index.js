...
<input type="file" id="second-package" accept=".zip, .bee_pack"></input>
	...
	/*pkgFile1.onchange = function () {
		for (x = 0; x < reader.readAsArrayBuffer(pkgFile1).length(); x++) {
			console.log(reader.readAsArrayBuffer(pkgFile1).files);
        }
	...
	mergeFile1.onchange = () => {
		for (var x = 0; x < mergeFile1.files.length; x++) {
			console.log(mergeFile1.FileReader.readAsArrayBuffer(Blob));
		}
	...
	mergeFile2.onchange = () => {
		for (var x = 0; x < mergeFile2.files.length; x++) {
			console.log(mergeFile2.FileReader.readAsArrayBuffer(Blob));
		}
		console.log("is changed")
	};
	if (setFiles = 2) {
...
console.log("booting up version 2.7a1.6 N/A 1.4")
